...
/* 计算菜单中各图标坐标值 */
void CalcBrowsePageMenusLayout(PT_PageLayout ptPageLayout)
{
...
int BrowsePageGetInputEvent(PT_PageLayout ptPageLayout, PT_EventOpr ptInputEvent)
{
...
static void ShowBrowsePage(PT_PageLayout ptPageLayout)
{
	...
	/* 1. 获得显存 */
	ptVideoMem = GetFreeVideoMem(ID("browse"), 1);
	if (ptVideoMem == NULL)
	{
...
static T_PageOpr g_tBrowsePageAction = {
	.name          = "browse",
	.Run           = BrowsePageRun,
...
{
	return RegisterPageOpr(&g_tBrowsePageAction);
}
